...
class ShowView(LoginRequired, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comments = self.get_comments_for_spot(spot, user)
        total_comments = len(comments)
        average_rating = Comment.objects.filter(spot=spot).aggregate(Avg('value'))['value__avg'] if total_comments > 0 else 0
        form = CommentForm()
        ...
        context.update({
            ...
            'user_comment_exists': user_comment_exists,  # 将该信息传递给模板
        })
